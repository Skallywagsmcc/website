...
//        Must have page and name otherwise nothing
        if (!empty($page) || !empty($array)) {
            ...
            try {
                ...
                $mail->Host = "mail.skallywags.club";                    //Set the SMTP server to send through
                $mail->SMTPAuth = true;                                   //Enable SMTP authentication
                $mail->Username = " mail@skallywags.club";              //SMTP username
                $mail->Password = "Bennyboo2k11";                              //SMTP password
                $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;         //Enable TLS encryption; `PHPMailer::ENCRYPTION_SMTPS` encouraged
                $mail->Port = 465;        //TCP port to connect to, use 465 for `PHPMailer::ENCRYPTION_SMTPS` above

                //Recipients
                $mail->setFrom("mbamber1986@gmail.com", $name);
